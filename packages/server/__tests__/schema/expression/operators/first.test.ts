...
const valuesTable: ValueTest[] = [
    ...
    { value: ["$path", "$path"], message: 'should invalidate with $path array > 1 value', exceptions: {
        ...
        zod: { code: "invalid_union", message: 'Invalid input' }} },
    { value: [{ $first: "$path" }], message: 'should invalidate with nested operator array', exceptions: { 
        zod: { code: "invalid_union", message: 'Invalid input' }
    } },
    { value: [{ $first: "$path" }, { $first: "$path" }], message: 'should invalidate with nested operator array', exceptions: {
        mongodb: { code: 16020, codeName: "Location16020", message: 'takes exactly 1 arguments. 2 were passed in.' },
        zod: { code: "invalid_union", message: 'Invalid input' }} },
    ...
    { value: { $first: "$path" }, message: 'should validate with known operator', exceptions: { } },
...
describe('expression operators - $first', () => {
    let database: Db;
    ...
    describe('validation', () => {
        test.each(valuesTable)('$message', async ({ value, exceptions }) => runTest({ $first: value }, exceptions, database));
    });
});
