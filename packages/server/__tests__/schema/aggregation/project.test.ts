...
import { projectionSchema } from '#src/schema/aggregation/project.js';
import { MongalayerCollectionType } from '#src/index.js';
import { exampleObject1, FilterTest } from '#test/data/filterTest.js';
...
const valuesTable: ValueTest[] = [
    { value: 1, message: 'should invalidate with number', exceptions: {
        mongodb: { code: 15969, codeName: "Location15969", message: "$project specification must be an object" },
    ...
    { value: {}, message: 'should invalidate with empty object', exceptions: {
        mongodb: { code: 51272, codeName: "Location51272", message: "projection specification must have at least one field" },
        zod: { code: 'invalid_value', message: 'Projection cannot be empty.' }
    } },
    { value: { $avg: 1 }, message: 'should invalidate with root operator', exceptions: {
        mongodb: { code: 16410, codeName: "Location16410", message: "FieldPath field names may not start with '$'" },
    ...
    { value: { prop: "a" }, message: 'should validate with prop -> string', exceptions: {} },
    { value: { prop: [] }, message: 'should validate with prop -> array', exceptions: {} },
    { value: { nested: { prop: "a", prop2: 1 } }, message: 'should validate with nested prop with string', exceptions: {} },
    { value: { nested: { prop: "$a", prop2: 1 } }, message: 'should validate with nested prop with path', exceptions: {} },
    { value: { nested: { prop: { $avg: ["$a"] }, prop2: 1 } }, message: 'should invalidate with nested prop with operator', exceptions: {
        zod: { code: 'invalid_union', message: 'Invalid input' }
    } },
    { value: { nested: { prop: { $x: ["$a"] }, prop2: 1 } }, message: 'should invalidate with nested prop with unknown operator', exceptions: {
        mongodb: { code: 31325, codeName: "Location31325", message: "Unknown expression $x" },
        zod: { code: 'invalid_union', message: 'Invalid input' }
    } },
    { value: { prop: [{ prop: "$a" }] }, message: 'should validate with nested prop in array', exceptions: {} },
];

const dbTestSoloTable: DbPipelineProjectTest[] = [
    { pipeline: [{$project: { name: 1 } }], properties: { present: [ "_id", "name" ], missing: [] }, message: 'include name' },
    { pipeline: [{$project: { name: 0 } }], properties: { present: [ "_id" ], missing: [ "name" ] }, message: 'exclude name' },
    { pipeline: [{$project: { name: 1, _id: 0 } }], properties: { present: [ "name" ], missing: [ "_id" ] }, message: 'include name, exclude _id' },
    { pipeline: [{$project: { _id: 0, name: 1 } }], properties: { present: [ "name" ], missing: [ "_id" ] }, message: 'exclude _id, include name' },
    { pipeline: [{$project: { "details.description": 1 } }], properties: { present: [ "_id", "details.description" ], missing: [ "details.metadata" ] }, message: 'include details.description - dot notation' },
    { pipeline: [{$project: { "details.description": 1, "details.metadata": 1 } }], properties: { present: [ "_id", "details.description", "details.metadata" ], missing: [  ] }, message: 'include details.description & details.metadata - dot notation' },
    { pipeline: [{$project: { details: { description: 1 } } }], properties: { present: [ "_id", "details.description" ], missing: [ "details.metadata" ] }, message: 'include details.description - object notation' },
    { pipeline: [{$project: { details: { description: 1, metadata: 1 } } }], properties: { present: [ "_id", "details.description", "details.metadata" ], missing: [  ] }, message: 'include details.description & details.metadata - object notation' },
    { pipeline: [{$project: { "details.nestedObject": 0 } }], properties: { present: [ "_id", "details.description", "details.metadata" ], missing: [ "details.nestedObject" ] }, message: 'exclude details.nestedObject - dot notation' },
    { pipeline: [{$project: { details: { nestedObject: 0 } } }], properties: { present: [ "_id", "details.description", "details.metadata" ], missing: [ "details.nestedObject" ] }, message: 'exclude details.nestedObject - object notation' },
    // Path projections
    { pipeline: [{$project: { name: 1, nameCopy: "$name" } }], properties: { present: [ "_id", "name", { prop: "nameCopy", value: exampleObject1.name } ], missing: [] }, message: 'project nameCopy' },
    {   
        pipeline: [{$project: { x: { 
            prop1: "$details.nestedObject.property1",
            prop2: "$details.nestedObject.property2"
        } } }], 
        properties: { present: [ "_id", { prop: "x", value: { 
            prop1: exampleObject1.details.nestedObject.property1, 
            prop2: exampleObject1.details.nestedObject.property2 
        } } ], missing: [] }, 
        message: 'project data object' 
    },
    {   
        pipeline: [{$project: { x: [ 
            { prop1: "$details.nestedObject.property1" },
...
describe('projection', () => {
    ...
    describe('validation', () => {
        test.each(valuesTable)('$message', async ({ value, exceptions }) => {
            ...
            try {
                const result = await database.collection<SchemaTest>("schemaTest").aggregate([{ $match: {} }, { $project: value }]).toArray();
    ...
    describe('on filterTestSolo collection', () => {
        test.each(dbTestSoloTable)('$message', async ({ pipeline, properties, message }) => {
            ...
            expect(mongaResult).toHaveLength(1);
