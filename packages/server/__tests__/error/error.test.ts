...
import { MongalayerCollectionType, ServerError, ServerErrorType, ServerErrorCode, DatabaseErrorCode, AuthorizationErrorCode, ValidationErrorCode } from "#src/index.js";
...
expect.extend({
    toThrowMongalayerError (actual: any, expected: { type: ServerErrorType, message: string, code: ServerErrorCode<typeof expected.type> }) {
        const { isNot } = this
        return {
            // do not alter your "pass" based on isNot. Vitest does it for you
            pass: actual instanceof ServerError && actual.type === expected.type && actual.message === expected.message && actual.code === expected.code,
...
interface CustomMatchers<R = unknown> {
    toThrowMongalayerError: (expected: { type: ServerErrorType, message: string, code: ServerErrorCode<typeof expected.type> }) => R
}
...
describe("Error - Database", () => {
    ...
    test("Duplicate", async () => {
        ...
        }, [], { document: AccessPermissions.ReadWrite })).rejects.toThrowMongalayerError({ type: ServerErrorType.Database, message: "Duplicate key error", code: DatabaseErrorCode.DuplicateKey });
    });
});
...
describe("Error - Authorization", () => {
    ...
    test("Insert - No permission", async () => {
        ...
        }, [], {})).rejects.toThrowMongalayerError({ type: ServerErrorType.Authorization, message: "Unauthorized documents found", code: AuthorizationErrorCode.UnauthorizedInsert });
    });
    ...
    test("Update - No permission", async () => {
        ...
        }, [], {})).rejects.toThrowMongalayerError({ type: ServerErrorType.Authorization, message: "Unauthorized documents found", code: AuthorizationErrorCode.UnauthorizedUpdate });
    });
});
...
describe("Error - Validation", () => {
    ...
    test("Invalid field", async () => {
        ...
        }, [], { document: AccessPermissions.ReadWrite })).rejects.toThrowMongalayerError({ type: ServerErrorType.Validation, message: "Failed to validate action payload", code: ValidationErrorCode.Unknown });
    });
});