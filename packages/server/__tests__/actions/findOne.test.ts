...
describe('Find One', () => {
    ...
    test("filter - $text.[idontexistprop] _id", async () => {
        try {
            ...
            const zodError = (e as z.ZodError), zodIssue = (zodError.issues[0] as $ZodIssueUnrecognizedKeys);
            
            expect(zodIssue.code).toBe("unrecognized_keys");
            expect(zodIssue.keys).toStrictEqual(["idontexistprop"]);
        }
    });
    ...
    test("filter - $text.$search missing", async () => {
        try {
            ...
            const zodError = (e as z.ZodError), zodIssue = (zodError.issues[0] as $ZodIssueInvalidType);
            
            expect(zodIssue.code).toBe("invalid_type");
            expect(zodIssue.expected).toBe("string");
        }
    });
});