...
export default async function <TSchema extends Document> (database: Db, accessService: UpdateAccessService, payload: FindOneAndUpdatePayload<TSchema>): Promise<FindOneAndUpdateReturnType<TSchema>> {
    ...
    if (documentsToUpdate.length === 0) {
        ...
        if (payload.options?.upsert === true) {
            const { doc: insertableDoc, role: insertableRole } = await accessService.getUpsertDocument(payload.filter, payload.update as UpdateSchema);
        ...
        filter = accessService.getFinalUpdateFilter({ _id: documentsToUpdate[0] }, payload.filter, payload.update as UpdateSchema);
