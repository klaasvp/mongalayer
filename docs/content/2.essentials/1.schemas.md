...
When using `$set` or `$unset` in updates, Mongalayer validates the update values against the relevant parts of your schema. For `$unset`, it verifies that the field is marked as `optional()` in the schema before allowing removal. For `$push`, the appended element (or each element provided via the `$each` modifier) is validated against the array's element schema, and `$pull` verifies that the target field is an array in the schema.
