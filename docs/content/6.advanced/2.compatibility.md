...
| `$lookup` | Joins documents from another collection, only a basic form is supported ([see below](#lookup-lookup)) |
...
::warning
Only the basic (equality match) form of `$lookup` is supported for now, due to the added complexity of applying access control to joined collections. The subquery form (`let` / `pipeline`) is not supported.
::
