...
| `$push` | Appends a single element to an array field (modifiers such as `$each`, `$position`, `$sort` and `$slice` are not supported) |
...
| `$lookup` | Joins documents from another collection, with limited pipeline support ([see below](#lookup-lookup)) |
| `$search` | Atlas Search integration ([see below](#atlas-search-search)) |
...
::warning
Only the equality match and limited pipeline stages are supported in `$lookup` for now, due to the added complexity of applying access control to joined collections. The `let` form is not supported.
::

The `$lookup` stage supports the following fields:
...
| `pipeline` | Optional pipeline applied to the joined documents ([see supported stages below](#pipeline-support)) |
| `as` | The name of the output array field |

```ts
const projectWithAssets = await projects.aggregate([
    { $match: { _id: projectId } },
    {
        $lookup: {
            from: "assets",
...
The optional `pipeline` field runs additional stages on the joined documents. The following stages are supported within a `$lookup` pipeline:

| Stage | Description |
|---|---|
| `$match` | Filters the joined documents |
| `$project` | Reshapes the joined documents |
| `$skip` | Skips a specified number of joined documents |
| `$limit` | Limits the number of joined documents |

```ts
const projectWithRecentAssets = await projects.aggregate([
    { $match: { _id: projectId } },
    {
        $lookup: {
            from: "assets",
