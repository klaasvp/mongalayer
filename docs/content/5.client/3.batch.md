...
Instead of constructing batches by hand, you can let the client batch operations for you. When [`autoBatch`](/client/configuration#autobatch) is enabled, supported read operations (`find`, `findOne`, `aggregate`) called through the normal collection API are queued and combined into a single batch request automatically.
...
Operations are collected for a short window — controlled by [`autoBatchDelay`](/client/configuration#autobatchdelay) (default `10` ms) — and then flushed together. Only the batchable read operations participate; all other operations are sent as individual requests.
