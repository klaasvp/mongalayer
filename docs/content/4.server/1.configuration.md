...
Check out the AWS Lambda / Express examples in [Quick start](/getting-started/usage#expose-over-http) for how to integrate with your HTTP server.
...
Mongalayer provides three methods for executing operations:

### `executeJSON(stringifiedBody, accessPayload) => string`

::tip
**This is the preferred method for HTTP integrations.** It validates the whole body payload by default and supports both regular (single) and [batch](/client/batch) operations through the same entry point.
::

Accepts a JSON-stringified body and returns a JSON string result. Unlike `execute`, it supports both regular (single) and batch operations through the same entry point. It handles serialization of special types like `Date` and `RegExp`. You can directly pass the HTTP request body, coming from the Mongalayer client, to this method. The body is either a single `{ action, payload }` object or an array of them for batch operations.

#### Single operation example
```ts
const singleBody = `{
    "action":{"database":"myapp","collection":"projects","operation":"findOne"},
    "payload":{"filter":{"_id":"project-123"}}
}`;

const single = await layer.executeJSON(singleBody, { user: { id: "user-123" } });
```
#### Batch operation example
```ts
// Batch of read operations — results are returned in order
const batchBody = `[
    {
        "action": { "database": "myapp", "collection": "projects", "operation": "findOne" },
        "payload": { "filter": { "_id": "project-123" } }
    ...
    {
        "action": { "database": "myapp", "collection": "assets", "operation": "find" },
        "payload": { "filter": { "projectID": "project-123" }, "options": { "limit": 10 } }
    }
]`;

const batch = await layer.executeJSON(batchBody, { user: { id: "user-123" } });
```

::note
Batch bodies may only contain read operations (`findOne`, `find`, `aggregate`). A batch containing a write operation is rejected during validation.
::

### `execute(action, stringifiedPayload, accessPayload) => string`

Executes a single action. Accepts a JSON-stringified payload and returns a JSON string result. Handles serialization of special types like `Date` and `RegExp`. Prefer `executeJSON` unless you specifically need to pass the action and payload separately, for instance when using the `routed` format.
...
::warning
When using `executeRaw`, you must ensure that the payload is properly formatted and does not contain unsupported types. We advice you to use `executeJSON` when using the client SDK.
::
...
Mongalayer already validates the structure of the action structure when calling `executeJSON`, `execute` or `executeRaw`. 
...
The third argument to `executeJSON` / `execute` / `executeRaw` is the **access payload** — a free-form object that becomes available in your access filters via the `%%` prefix.
